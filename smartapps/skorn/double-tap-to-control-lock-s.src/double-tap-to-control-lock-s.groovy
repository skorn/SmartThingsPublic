 ...
 */
definition(
    name: "Double Tap to Control Lock(s)",
    namespace: "skorn",
    author: "Martin Lariz",
    description: "Control a door lock when switch is double tapped.",
...
preferences {
	...
	section("Lock or unlock all of these doors") {
		input "doors", "capability.lock", multiple: true, required: false
	}
	section("And lock but not unlock all of these doors") {
		input "lockDoors", "capability.lock", multiple: true, required: false
	}
	section("And unlock but not lock all of these doors") {
		input "unlockDoors", "capability.lock", multiple: true, required: false
	}
}
...
def switchHandler(evt) {
	...
	if (evt.physical) {
		if (evt.value == "on" && lastTwoStatesWere("on", recentStates, evt)) {
			log.debug "detected two taps, locking door(s)"
			lockDoors()*.lock()
		} else if (evt.value == "off" && lastTwoStatesWere("off", recentStates, evt)) {
			log.debug "detected two taps, unlocking door(s)"
			unlockDoors()*.unlock()
		}
	}
	else {
...
private lockDoors() {
	(doors + lockDoors).findAll{it}
}

private unlockDoors() {
	(doors + unlockDoors).findAll{it}
}
